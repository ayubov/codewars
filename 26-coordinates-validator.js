...
const isValidCoordinates = (coordinates) => {
  const first = +coordinates.slice(0, coordinates.indexOf(','));
  const second = +coordinates.slice(coordinates.indexOf(',') + 1);
  const letters = /[a-z]/i.test(coordinates);
  return Math.abs(first) < 90 && Math.abs(second) < 180 && !letters;
};
