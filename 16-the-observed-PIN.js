...
You can enter an unlimited amount of wrong PINs,they neverfinally lock the system or sound
the alarm. That's why we can try out all possible (*) variations.
* possible in sense of: the observed PIN itself and all variations considering the adjacent digits

Can you help us to find all those variations? It would be nice to have a function, that returns
an array of all variations for an observed PIN with a length of 1 to 8 digits. We could name the
function getPINs (get_pins in python). But please note that all PINs, the observed one and also
the results, must be strings, because of potentially leading '0's.
...
export default (observed) => {
  ...
  const iter = (list, acc, acc2) => {
    ...
    const currentList = list.shift();
    return iter(list, adding(compliance.get(Number(currentList)), acc, acc2), acc2 + 1);
  };

  const unsortedResult = iter(arr, firstAcc, 0);
  return unsortedResult.filter((item, pos) =>
    (unsortedResult.indexOf(item) === pos) && (item.length === observed.length));
};
